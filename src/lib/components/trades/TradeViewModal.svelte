...
<div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" on:click={close}>
    <div class="bg-light-card dark:bg-dark-card rounded-lg shadow-xl max-w-xl w-full" on:click|stopPropagation>
        ...
        <div class="p-4 border-b border-light-border dark:border-dark-border">
            <div class="flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <h2 class="text-xl font-semibold text-light-text dark:text-dark-text">{trade.symbol}</h2>
        ...
        <div class="p-6 space-y-6">
            ...
            {#if trade.image}
                <div>
                    <h3 class="text-sm font-medium text-light-text-muted dark:text-dark-text-muted mb-2">Trade Image</h3>
                    <img 
                        src={`data:${trade.image.contentType};base64,${arrayBufferToBase64(trade.image.data)}`} 
                        alt="Trade" 
